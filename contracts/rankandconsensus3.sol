...
function determineConsensus(uint256 _communityId, uint256 _weekNumber, uint256 _groupId) public {
        ...
        // Ensure the consensusRanking array has the correct length
        require(consensusRanking.length == groupSize, "Invalid consensus ranking length");
    ...
    function calculateTransientScore(uint256 _communityId, uint256 _weekNumber, uint256 _groupId, uint256 memberIndex, ICommunityGovernanceContributions.Group memory group) private view returns (uint256) {
        ...
        uint256 variance = calculateVariance(memberRankings, meanRanking);
    ...
    function calculateVariance(uint256[] memory values, uint256 mean) private pure returns (uint256) {
        uint256 sumSquaredDiff = 0;
        for (uint256 i = 0; i < values.length; i++) {
            if (values[i] * SCALE > mean) {
                uint256 diff = values[i] * SCALE - mean;
                sumSquaredDiff += (diff * diff) / SCALE;
            } else {
                uint256 diff = mean - values[i] * SCALE;
                sumSquaredDiff += (diff * diff) / SCALE;
            }
        }
        return (sumSquaredDiff * SCALE) / values.length;
    }
