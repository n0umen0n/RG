    ...
    for (let groupId = 0; groupId < groups.length; groupId++) {
        ...
        for (const member of members) {
            try {
                ...
                console.log(`Submitted ranking by ${member}:`);
                ranking.forEach((rankedMember, index) => {
                    console.log(`  Rank ${index + 1}: ${rankedMember}`);
                });
            } catch (error) {
...
async function determineConsensus(communityId) {
    ...
    for (let groupId = 0; groupId < groups.length; groupId++) {
        ...
        // Retrieve and log each member's rankings
        for (const member of groupMembers) {
            const ranking = await communityGovernanceRankings.methods.getRanking(communityId, currentWeek, groupId, member).call();
            rankings.push({ member, ranking });

            console.log(`Ranking submitted by ${member} in group ${groupId}:`);
            ranking.forEach((rankedMember, index) => {
                console.log(`  Rank ${index + 1}: ${rankedMember}`);
            });
        }

        // Calculate and log the mean, variance, and max variance for each member
        try {
            const transientScoresData = await communityGovernanceRankings.methods.getTransientScores(communityId, currentWeek, groupId).call();
            const addresses = transientScoresData[0];
            const transientScores = transientScoresData[1];

            console.log(`\nTransient Scores for group ${groupId}:`);
            for (let i = 0; i < addresses.length; i++) {
                console.log(`  Address: ${addresses[i]}, Score: ${transientScores[i]}`);
            }
        } catch (error) {
            console.log(`Failed to retrieve transient scores for group ${groupId}. Reason: ${error.message}`);
        }

        // Determine consensus and log final ranking
        try {
            ...
            for (let i = 0; i < consensusRanking[0].length; i++) {
                console.log(`  Rank ${i + 1}: Address: ${consensusRanking[0][i]}, Score: ${consensusRanking[1][i]}`);
            }
