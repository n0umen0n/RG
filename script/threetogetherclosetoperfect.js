      ...
      for (let groupId = 0; groupId < groups.length; groupId++) {
        ...
        for (const member of members) {
          try {
            ...
            let ranking = Array.from({length: members.length}, (_, i) => i + 1);
            ...
            console.log(`Submitted ranking by ${member}:`, ranking);
            allRankings[groupId][member] = ranking;
          } catch (error) {
    ...
    async function determineAndLogConsensus(communityId, allRankings) {
      ...
      for (let groupId = 0; groupId < groups.length; groupId++) {
        try {
          ...
          // Log group members before determining consensus
          console.log(`Group ${groupId} members:`, groups[groupId].members);
          ...
          for (let i = 0; i < groupMembers.length; i++) {
            console.log(`  ${i + 1}. ${groupMembers[i]} - Rank: ${rankedScores[i]}, Transient Score: ${transientScores[i]}`);
          }
          console.log('\n');
          ...
          for (let i = 0; i < groupMembers.length; i++) {
            const memberRankings = Object.values(groupRankings).map(ranking => ranking[i]);
            ...
            console.log(`Member ${groupMembers[i]}:`);
          ...
          for (let i = 0; i < groupMembers.length; i++) {
            ...
            const actual = rankedScores[groupMembers.indexOf(expected.member)];
          ...
          try {
            const groupMembers = groups[groupId].members;
            console.log(`Group ${groupId} members:`, groupMembers);
