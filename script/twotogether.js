  ...
  for (let groupId = 0; groupId < groups.length; groupId++) {
    ...
    for (const member of members) {
      try {
        ...
        await communityGovernanceContributions.methods.submitRanking(communityId, currentWeek, groupId, ranking)
    ...
    async function determineConsensus(communityId) {
      ...
      for (let groupId = 0; groupId < groups.length; groupId++) {
        try {
          await communityGovernanceContributions.methods.determineConsensus(communityId, currentWeek, groupId)
            .send({ from: deployerAccount, gas: 3000000 });
          console.log(`Consensus determined for group ${groupId}`);

          // Get and log consensus ranking
          const consensusRanking = await communityGovernanceContributions.methods.getConsensusRanking(communityId, currentWeek, groupId).call();
