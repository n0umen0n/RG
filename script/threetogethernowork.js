(async () => {
  try {
    ...
    const membersPerCommunity = [10, 20, 25]; // Adjusted for each community
    ...
    for (let communityId = 1; communityId <= communities.length; communityId++) {
      ...
      for (let j = 0; j < membersPerCommunity[communityId - 1]; j++) {
    ...
    // Approve users
    for (let communityId = 1; communityId <= communities.length; communityId++) {
      const approvers = usableAccounts.slice(0, 5); // Use the first five accounts as approvers
