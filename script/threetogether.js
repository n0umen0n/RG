(async () => {
  ...
  for (let groupId = 0; groupId < groups.length; groupId++) {
    ...
    for (const member of sortedMembers) {
      try {
        // Create a random ranking of group members (scores from 1 to group size)
        let ranking = Array.from({length: sortedMembers.length}, (_, i) => i + 1);
        ranking.sort(() => Math.random() - 0.5);

        // Create a mapping of members to their rankings
        const rankingMap = {};
        sortedMembers.forEach((m, index) => {
          rankingMap[m] = ranking[index];
        });

        // Convert the ranking map back to an array in the sorted order
        const sortedRanking = sortedMembers.map(m => rankingMap[m]);

        await communityGovernanceRankings.methods.submitRanking(communityId, currentWeek, groupId, sortedRanking)
          .send({ from: member, gas: 3000000 });
        console.log(`Ranking submitted by ${member} for group ${groupId}`);

        // Log the submitted ranking for verification
        console.log(`Submitted ranking by ${member}:`, sortedRanking);
        allRankings[groupId][member] = sortedRanking;
      } catch (error) {
  ...
  return allRankings;
    ...
    async function determineAndLogConsensus(communityId, allRankings) {
  ...
  for (let groupId = 0; groupId < groups.length; groupId++) {
    try {
      console.log(`Determining consensus for group ${groupId}`);
      
      // Sort group members to ensure consistent order
      const sortedGroupMembers = [...groups[groupId].members].sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
      
      console.log(`Group ${groupId} members:`, sortedGroupMembers);

      // Log all rankings submitted for this group
      for (const member of sortedGroupMembers) {
        const ranking = await communityGovernanceRankings.methods.getRanking(communityId, currentWeek, groupId, member).call();
        console.log(`Ranking submitted by ${member}:`, ranking);
      }

      const gasEstimate = await communityGovernanceRankings.methods.determineConsensus(communityId, currentWeek, groupId).estimateGas({ from: deployerAccount });
      const result = await communityGovernanceRankings.methods.determineConsensus(communityId, currentWeek, groupId)
      ...
      console.log(`Consensus ranking for group ${groupId}:`, consensusRanking);

      const rankedScores = consensusRanking.rankedScores || consensusRanking[0];
      const transientScores = consensusRanking.transientScores || consensusRanking[1];
      const timestamp = consensusRanking.timestamp || consensusRanking[2];

      console.log('Ranked Scores:', rankedScores);
      console.log('Transient Scores:', transientScores);
      console.log('Timestamp:', timestamp);

      console.log('Final Ranking:');
      for (let i = 0; i < sortedGroupMembers.length; i++) {
        console.log(`  ${i + 1}. ${sortedGroupMembers[i]} - Rank: ${rankedScores[i]}, Transient Score: ${transientScores[i]}`);
      }
      console.log('\n');

      // Log individual transient scores for verification
      const transientScoresResult = await communityGovernanceRankings.methods.getTransientScores(communityId, currentWeek, groupId).call();
      console.log(`Transient scores for group ${groupId}:`, transientScoresResult);

      const members = transientScoresResult[0];
      const scores = transientScoresResult[1];

      if (members && scores && members.length === scores.length) {
        for (let i = 0; i < members.length; i++) {
          console.log(`  ${members[i]} - Score: ${scores[i]}`);
        }
      } else {
        console.log('Unexpected transient scores format:', transientScoresResult);
      }

      // Verify consensus ranking
      console.log('\nVerifying Consensus Ranking:');
      const groupRankings = allRankings[groupId];
      const expectedTransientScores = [];
      
      for (let i = 0; i < sortedGroupMembers.length; i++) {
        const member = sortedGroupMembers[i];
        const memberRankings = sortedGroupMembers.map(m => groupRankings[m][i]);
        const mean = memberRankings.reduce((a, b) => a + b, 0) / memberRankings.length;
        const variance = calculateVariance(memberRankings);
        const maxVariance = calculateMaxVariance(sortedGroupMembers.length);
        const consensusTerm = 1 - (variance / maxVariance);
        const expectedTransientScore = mean * consensusTerm;
        expectedTransientScores.push(expectedTransientScore);
        ...
        console.log(`  Expected Transient Score: ${expectedTransientScore}`);
        console.log(`  Actual Transient Score: ${transientScores[i]}`);
        console.log(`  Difference: ${Math.abs(expectedTransientScore - transientScores[i])}`);
      }

      const expectedRanking = expectedTransientScores
        .map((score, index) => ({ score, index }))
        .sort((a, b) => b.score - a.score)
        .map((item, index) => ({ member: sortedGroupMembers[item.index], rank: sortedGroupMembers.length - index }));
      
      let rankingCorrect = true;
      for (let i = 0; i < sortedGroupMembers.length; i++) {
        const expected = expectedRanking[i];
        const actual = rankedScores[sortedGroupMembers.indexOf(expected.member)];
      ...
      try {
        ...
        for (const member of sortedGroupMembers) {
          const ranking = await communityGovernanceRankings.methods.getRanking(communityId, currentWeek, groupId, member).call();
          console.log(`Ranking for ${member}:`, ranking);
        }
      } catch (stateError) {
        console.log('Failed to retrieve contract state:', stateError.message);
      }
    }
  }
}

    // Submit rankings and determine consensus for each community
    for (let communityId = 1; communityId <= communities.length; communityId++) {
      const allRankings = await submitRankings(communityId);
      await determineAndLogConsensus(communityId, allRankings);
    }
