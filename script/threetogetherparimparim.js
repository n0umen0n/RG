(async () => {
  try {
    ...
    // Function to determine consensus and log detailed calculations
    async function determineConsensus(communityId) {
      ...
      for (let groupId = 0; groupId < groups.length; groupId++) {
        try {
          ...
          const transientScores = await communityGovernanceRankings.methods.getTransientScores(communityId, currentWeek, groupId).call();
          console.log(`Transient scores for group ${groupId}:`);
          for (let i = 0; i < transientScores[0].length; i++) {
            console.log(`  ${transientScores[0][i]} - Score: ${transientScores[1][i]}`);
          }
        } catch (error) {
